...
---
这一节主要讲解Detectron，mmdetection和我们的maskrcnn_benchmark(ours)三种方法的比较
## Hardware
- `2 NVIDIA Titan X GPUs`: 我在实验是使用的是双核的Titan GPUs

## Software
- `ubuntu16.04/ubuntu18.04`: 现在使用ubuntu版本，windows下没有测试
- `PyTorch 1.0`: 基于pytorch1.0，这里pytorch1.0相比与之前pytorch0.4版本，安装起来更加方便，而且能兼容大部分0.4的功能
- `CUDA 9.2`
- `CUDNN 7.1`
- `NCCL 2.2.13-1` ： 多GPUs进行并行计算，Caffe自带实现. 在多个 GPU 上运行 Caffe 需要使用 NVIDIA NCCL


## 端到端的Faster R-CNN 和 Mask R-CNN的基准
使用与Detectron相同的实验设置作训练（这里的Detectron指的是mask RCNN论文当中所指的代码），
我们用来自Caffe2的ImageNet 权重初始化检测模块，和Detectron一样。
预训练模型在model id中链接可以使用。
...
---
下面是Detectron，mmdetection和我们的maskrcnn_benchmark三种方法的比较，可以不看
---
